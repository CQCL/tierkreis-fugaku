...
command = (
    ...
    f"env OMP_NUM_THREADS={os.environ.get("OMP_NUM_THREADS", 10)} UV_PROJECT_ENVIRONMENT=compute_venv uv run main.py"
)
...
def pjsub_uv_executor(group_name: str, logs_path: Path) -> PJSUBExecutor:
    spec = JobSpec(
        ...
        account=group_name,
        command=command,
        resource=ResourceSpec(nodes=10, memory_gb=None, gpus_per_node=None),
        walltime="00:15:00",
        mpi=MpiSpec(max_proc_per_node=None, proc=5),
